...
int sleft[vertices,vertices]=...;
int sright[vertices,vertices]=...;
int sabove[vertices,vertices]=...;
int sbelow[vertices,vertices]=...;
int sdist = ...;
float sdd[vertices,vertices]=...;
...
dvar float+ averagex;
dvar float+ averagey;
dvar float+ abscenterx[vertices];
dvar float+ abscentery[vertices];

dvar float+ thecenter;

minimize 10*thestress+7*thesize+8*maxX+8*maxY+2*thecenter;

subject to {

  averagex <= sum(u in vertices)(xs[u]) / nv;
  averagex >= sum(u in vertices)(xs[u]) / nv;
  averagey <= sum(u in vertices)(ys[u]) / nv;
  averagey >= sum(u in vertices)(ys[u]) / nv;
  thecenter <= sum(u in vertices)(abscenterx[u]+abscentery[u]);
  thecenter >= sum(u in vertices)(abscenterx[u]+abscentery[u]);
  forall (u in vertices)
    abscenterx[u] <= maxl(0,averagex-xs[u],xs[u]-averagex);
  forall (u in vertices)
    abscenterx[u] >= maxl(0,averagex-xs[u],xs[u]-averagex);
  forall (u in vertices)
    abscentery[u] <= maxl(0,averagey-ys[u],ys[u]-averagey);
  forall (u in vertices)
    abscentery[u] >= maxl(0,averagey-ys[u],ys[u]-averagey);
  ...
  forall (u in vertices)yf[u]<=maxY;
    forall (u in vertices, v in vertices : sleft[u,v] != 0)
    xf[u] <= xs[v];
  forall (u in vertices, v in vertices : sright[u,v] != 0)
    xf[v] <= xs[u];
  forall (u in vertices, v in vertices : sabove[u,v] != 0)
    yf[v] <= ys[u];
  forall (u in vertices, v in vertices : sbelow[u,v] != 0)
    yf[u] <= ys[v];
    forall (u in vertices, v in vertices : dd[u,v]!=0) dist[u,v] <= sdist;
    forall(u in vertices,v in vertices : sdd[u,v]!=0) dist[u,v] <= sdd[u,v];
}
